...
// ==============================================
// DOM Element Getters (Works for both pages)
// ==============================================
...
// ==============================================
// Authentication Functions (Shared)
// ==============================================
...
async function handleAuthenticatedUser(user) {
    ...
    try {
        ...
        if (window.location.pathname.includes('admin.html')) {
            // Admin page logic
            if (!profile?.is_admin && user.email !== 'admin@cakeshop.com') {
            ...
            // Customer page logic
            // Check if admin trying to access customer page
            if (user.email === 'admin@cakeshop.com' && profile?.is_admin) {
            ...
            // Pre-fill order form
            if (profile && profile.full_name && elements.customerName) {
                elements.customerName.value = profile.full_name;
            } else if (user.user_metadata?.full_name && elements.customerName) {
                elements.customerName.value = user.user_metadata.full_name;
            }
            if (user.email && elements.customerEmail) {
...
function setupCustomerEventListeners() {
    ...
    if (elements.categoryBtns && elements.categoryBtns.length > 0) {
...
// ==============================================
// Shopping Bag Functions (Customer)
// ==============================================
function addToBag() {
    if (!currentProduct) {
...
function setupAdminEventListeners() {
    ...
    if (elements.productForm) {
        elements.productForm.addEventListener('submit', (e) => {
            ...
            handleAdminProductSave(e);
        });
    }
    
    if (elements.cancelEdit) {
        elements.cancelEdit.addEventListener('click', cancelProductEdit);
    }
    
    // Status modal
    if (elements.cancelStatus) {
        elements.cancelStatus.addEventListener('click', () => {
    ...
    if (elements.saveStatus) {
        elements.saveStatus.addEventListener('click', (e) => {
            e.preventDefault();
    ...
    if (elements.saveProduct) {
        elements.saveProduct.addEventListener('click', (e) => {
            e.preventDefault();
...
function switchTab(tab) {
    ...
    if (elements.tabContents) {
        Object.keys(elements.tabContents).forEach(key => {
            if (key === tab) {
                elements.tabContents[key].classList.remove('hidden');
                elements.tabContents[key].classList.add('animate-fade-in-up');
            } else {
                elements.tabContents[key].classList.add('hidden');
                elements.tabContents[key].classList.remove('animate-fade-in-up');
            }
        });
    }
...
async function loadRecentOrders() {
    try {
        ...
        elements.recentOrders.innerHTML = '<p class="text-red-500 text-center">Error loading orders</p>';
    }
}
...
async function loadTopProducts() {
    try {
        ...
        topProducts.forEach((product, index) => {
            const rankColors = ['bg-yellow-500', 'bg-gray-400', 'bg-yellow-700', 'bg-pink-500', 'bg-purple-500'];
            const rankColor = rankColors[index] || 'bg-gray-300';
            
            html += `
                <div class="flex items-center justify-between p-3 bg-white border rounded-lg hover:shadow-sm transition duration-200 animate-fade-in-up">
                    <div class="flex items-center">
                        <span class="inline-flex items-center justify-center w-8 h-8 rounded-full ${rankColor} text-white text-sm font-bold mr-3">
                            ${index + 1}
                        </span>
                        <div>
                            <div class="font-medium text-gray-800 truncate max-w-xs">${product.name}</div>
        ...
        elements.topProducts.innerHTML = '<p class="text-red-500 text-center">Error loading data</p>';
    }
}
...
function renderOrdersTable() {
    ...
    allOrders.forEach((order, index) => {
        ...
        tableHTML += `
            <tr class="table-row border-b hover:bg-gray-50 animate-fade-in-up" style="animation-delay: ${index * 0.05}s">
                <td class="p-4">
                ...
                <td class="p-4">
                    <span class="inline-flex items-center justify-center w-8 h-8 bg-pink-100 text-pink-800 rounded-full text-sm font-medium">
                        ${itemsCount}
                    </span>
                </td>
                ...
                <td class="p-4">
                    <div class="flex space-x-2">
                        <button class="update-status-btn action-btn bg-pink-600 hover:bg-pink-700 text-white px-3 py-1.5 rounded text-sm font-medium transition duration-300 flex items-center"
                                data-id="${order.id}"
                                data-status="${order.status}">
                            <i class="fas fa-edit mr-1"></i>
                            Update
                        </button>
                        <button class="view-order-btn action-btn bg-gray-200 hover:bg-gray-300 text-gray-800 px-3 py-1.5 rounded text-sm font-medium transition duration-300 flex items-center"
                                data-id="${order.id}">
    ...
    elements.ordersTable.innerHTML = tableHTML;
...
function showStatusModal(orderId, currentStatus) {
    console.log('Showing status modal for order:', orderId);
    currentOrderToUpdate = orderId;
    
    if (elements.newStatus) {
        elements.newStatus.value = currentStatus;
    }
    
    if (elements.statusModal) {
        elements.statusModal.classList.remove('hidden');
        elements.statusModal.classList.add('flex');
    }
}
...
async function updateOrderStatus() {
    ...
    const newStatus = elements.newStatus ? elements.newStatus.value : 'pending';
    
    console.log(`Updating order ${currentOrderToUpdate} to status: ${newStatus}`);
    ...
    try {
        ...
        if (elements.statusModal) {
            elements.statusModal.classList.add('hidden');
            elements.statusModal.classList.remove('flex');
        }
...
function showOrderDetailsModal(order) {
    ...
    const modalHTML = `
        <div class="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4 animate-fade-in">
            <div class="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden animate-slide-in-left">
                <div class="p-6 border-b bg-gradient-to-r from-pink-50 to-white">
                    <div class="flex justify-between items-center">
                        <div>
                ...
                <div class="p-6 overflow-y-auto max-h-[calc(90vh-120px)] scrollbar-admin">
                    <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <div class="space-y-3">
                                    <div>
                                        ...
                                        <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium ${statusClass} mt-1">
                                            <i class="fas fa-circle mr-2 text-xs"></i>
                                            ${order.status || 'pending'}
                                        </span>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <p class="font-medium">${new Date(order.updated_at || order.created_at).toLocaleString()}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="space-y-3">
                                    <div>
                                        ...
                                        <p class="font-medium">${order.customer_name || 'Not provided'}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-gray-600">Email</p>
                                        <p class="font-medium">${order.customer_email || 'Not provided'}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-gray-600">Phone</p>
                                        <p class="font-medium">${order.customer_phone || 'Not provided'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <h4 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Delivery Information</h4>
                            <div class="bg-gray-50 p-4 rounded-lg border">
                                <p class="font-medium text-gray-800">${order.delivery_address || 'No address provided'}</p>
                                ${order.delivery_date ? `
                        ...
                        <div>
                            <h4 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Order Items</h4>
                        ...
                        ${order.notes ? `
                        <div>
                            <h4 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Special Instructions</h4>
                        ...
                        <div class="border-t pt-6">
                            <div class="flex justify-between items-center">
                                <div>
                                    ...
                                    <p class="text-gray-600 text-sm mt-1">Including all items and taxes</p>
                                </div>
                                <div class="text-3xl font-bold text-pink-600">$${order.total_amount || 0}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 border-t bg-gray-50">
                    <button onclick="this.parentElement.parentElement.parentElement.remove()" 
                            class="w-full bg-pink-600 hover:bg-pink-700 text-white py-3 rounded-lg font-semibold transition duration-300">
                        Close Details
                    </button>
                </div>
            </div>
        ...
        elements.productsTable.innerHTML = `
            <tr>
                <td colspan="5" class="p-8 text-center text-gray-500">
                    <i class="fas fa-birthday-cake text-3xl text-gray-300 mb-2 block"></i>
    ...
    allProducts.forEach((product, index) => {
        tableHTML += `
            <tr class="table-row border-b hover:bg-gray-50 animate-fade-in-up" style="animation-delay: ${index * 0.05}s">
                ...
                <td class="p-4">
                    <span class="px-3 py-1.5 bg-pink-100 text-pink-800 rounded-full text-sm font-medium">
                        ${product.category || 'Uncategorized'}
                    </span>
                </td>
                <td class="p-4 font-semibold text-gray-800">$${product.price || 0}</td>
                <td class="p-4">
                    <span class="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium ${product.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">
                        <i class="fas fa-circle mr-2 text-xs"></i>
                ...
                <td class="p-4">
                    <div class="flex space-x-2">
                        <button class="edit-product-btn action-btn bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded text-sm font-medium transition duration-300 flex items-center"
                                data-id="${product.id}">
                            <i class="fas fa-edit mr-1"></i>
                            Edit
                        </button>
                        <button class="delete-product-btn action-btn ${product.is_active ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-400 hover:bg-gray-500'} text-white px-3 py-1.5 rounded text-sm font-medium transition duration-300 flex items-center"
                                data-id="${product.id}"
...
async function handleAdminProductSave(e) {
    ...
    // Get form values
    const name = elements.productName ? elements.productName.value.trim() : '';
    const description = elements.productDescription ? elements.productDescription.value.trim() : '';
    const price = elements.productPrice ? parseFloat(elements.productPrice.value) : 0;
    const category = elements.productCategory ? elements.productCategory.value : '';
    const imageUrl = elements.productImage ? elements.productImage.value.trim() : '';
    ...
    // Show loading state
    const saveBtn = elements.saveProduct;
...
async function deleteProduct(productId, productName) {
    ...
    if (!confirm(`Are you sure you want to delete "${productName}"?\n\nThis will mark the product as inactive and it will no longer appear in the shop.`)) {
        return;
    }
...
function renderUsersTable() {
    ...
    allUsers.forEach((user, index) => {
        // Skip current admin from the list to prevent self-ban
        if (user.id === currentUser?.id) return;
        
        tableHTML += `
            <tr class="table-row border-b hover:bg-gray-50 animate-fade-in-up" style="animation-delay: ${index * 0.05}s">
                <td class="p-4">
                    <div class="font-mono text-sm text-gray-600" title="${user.id}">
                ...
                <td class="p-4">${user.full_name || 'Not set'}</td>
                <td class="p-4 text-sm text-gray-600">
                    ${new Date(user.created_at).toLocaleDateString()}
                </td>
                <td class="p-4">
                    <div class="flex flex-wrap gap-2">
                        <span class="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium ${user.is_banned ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}">
                            <i class="fas fa-circle mr-2 text-xs"></i>
                ...
                <td class="p-4">
                    <div class="flex space-x-2">
                        ${!user.is_admin ? `
                        <button class="toggle-ban-btn action-btn ${user.is_banned ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'} text-white px-3 py-1.5 rounded text-sm font-medium transition duration-300 flex items-center"
                                data-id="${user.id}"
                        ...
                        <span class="text-gray-400 text-sm px-3 py-1.5">Admin User</span>
                        `}
                    </div>
                </td>
            </tr>
    ...
    elements.usersTable.innerHTML = tableHTML;
    ...
    setTimeout(() => {
        document.querySelectorAll('.toggle-ban-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                const isCurrentlyBanned = e.target.closest('button').dataset.banned === 'true';
                const userEmail = e.target.closest('button').dataset.email;
                
                const action = isCurrentlyBanned ? 'unban' : 'ban';
                const actionText = isCurrentlyBanned ? 'unban' : 'suspend';
                
                if (!confirm(`Are you sure you want to ${actionText} ${userEmail}?\n\nThis will ${isCurrentlyBanned ? 'allow' : 'prevent'} them from logging in.`)) {
                    return;
                }
                ...
                try {
                    ...
                    showNotification(`User ${isCurrentlyBanned ? 'unbanned' : 'banned'} successfully`, 'success');
...
async function handleLogout() {
    ...
    try {
        ...
        // Clear shopping bag if on customer page
        if (!window.location.pathname.includes('admin.html')) {
            shoppingBag = [];
...
async function handleOrderSubmit(e) {
    ...
    try {
        ...
        // Reload orders if on customer page
        if (!window.location.pathname.includes('admin.html')) {
            await loadUserOrders();
        }
...
function setupRealtimeSubscriptions() {
    ...
    // Only setup customer subscriptions if on customer page
    if (!window.location.pathname.includes('admin.html')) {
